...
model_dir = os.curdir + '\\'
data_dir = os.curdir + '\\'
error_season = {'start':datetime(2000, 4, 1), 'end':datetime(2000, 8, 31)}
...
'DWR_tw':{
'gauge_type':'Mix',
'data_file':'DWQI_TDG_Temp_2011-2015.csv',
'mixed_outlet_file':'dwo_45.opt',
'spill_bays' : '''
dwo_gate3_seg45.opt
dwo_gate4_seg45.opt
dwo_gate5_seg45.opt
...
dwo_gate8_seg45.opt
''',
'plot_name':'TDG_TW_Compare_DWR_2011-2015',
'title':'Dworshak Tailwater TDG (DWQI) 2011-2015'
},\
'LWG_tw':{
'gauge_type':'TDGgauge',
...
'surface_outlet_file':'BON_tsr_1_seg76.opt',
'deep_outlet_file':'BON_tsr_2_seg76.opt',
...
if 'power_gates' in projects[project]:
    power_gates = projects[project]['power_gates']
    pwr_files = power_gates.split()
...
if projects[project]['gauge_type'] == 'TDGgauge':
    # get the spillway TDG first
    sb_dt = {}
    for sb in sb_files:
        model_result = W2_generic_read(model_dir + sb,2,csv = True)
        #reorg
    ...
    #get the powerhouse concentration
    power_dt = {}
    for pwr in pwr_files:
        model_result = W2_generic_read(model_dir + pwr, 2, 8, print_errors=True, csv = True)
        for jday in model_result.keys():
            dt = datetime(year-1,12,31)+timedelta(jday)
            if dt in power_dt:
                if model_result[jday]['  TDG(%)'] != -99.0:
                    power_dt[dt] = model_result[jday]['  TDG(%)']
            else:
                power_dt[dt] = model_result[jday]['  TDG(%)']

    #get the total flow for TDG gauge calculation
    model_result = W2_generic_read(model_dir+projects[project]['model_flow_out'], 2, 8, print_errors=True, csv = True)
    qot_dt = {}
    for jday in model_result.keys():
        dt = datetime(year-1,12,31)+timedelta(jday)
        qot_dt[dt] = model_result[jday]['QWD(m3s-1)']
    temp_dt = {}
    SP_Mix_ratio = {}
    for dt in qot_dt.keys():
        if sb_dt[dt] == -99.0:
            temp_dt[dt] = mix_dt[dt]
            sb_dt[dt] = mix_dt[dt]
        else:
            q_kcfs = qot_dt[dt] / 0.0283 / 1000
            SP_Mix_ratio[dt] = 1/(1+np.exp(-1*(q_kcfs-projects[project]['f1'])/projects[project]['f2']))
            temp_dt[dt] = SP_Mix_ratio[dt]*sb_dt[dt] + (1-SP_Mix_ratio[dt])*mix_dt[dt]
    model_dt, model_TDGgauge = format_dict_date_to_list_for_plot(temp_dt)
    model_dt, model_TDGsp = format_dict_date_to_list_for_plot(sb_dt)
    model_dt, model_TDGmix = format_dict_date_to_list_for_plot(mix_dt)
    model_dt, model_TDGpower = format_dict_date_to_list_for_plot(power_dt)
...
for dt in meas_dt:
    ...
    diffs += [mod-meas]
    if datetime(2000, dt.month, dt.day) >= error_season['start'] and datetime(2000, dt.month, dt.day) <= error_season['end']:
        diffs_subset+= [mod-meas]
    mod_vals += [mod]
    obs_vals += [meas]
...
tstat_data = model_dir + 'TDG_TW_allStats_' + str(year) + '.csv'
...
fig.autofmt_xdate()
ax1.plot(meas_dt, meas_wsel,label = 'Measured TDG', linestyle='None', marker='.', color='r', markersize=1)
if projects[project]['gauge_type'] == 'TDGgauge':
    ax1.plot(model_dt[1:], model_TDGgauge[1:], label = 'Modeled TDG', color = 'k')
...
x_range = [min(model_dt), max(model_dt)]
