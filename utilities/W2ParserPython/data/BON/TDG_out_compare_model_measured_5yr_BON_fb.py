...
'surface_outlet_file':'LWG_tsr_1_seg43.opt',
'deep_outlet_file':'LWG_tsr_10_seg43.opt',
'mixed_outlet_file':'LWG_tsr_1_seg43.opt',
...
model_result = W2_generic_read(model_dir+projects[project]['surface_outlet_file'], 0, print_errors=True, csv = True)
TDG_aaa_dt = {}
for jday in model_result.keys():
    dt = datetime(year-1,12,31)+timedelta(jday)
    TDG_aaa_dt[dt] = model_result[jday]['  TDG(%)']
model_dt, model_TDGsurface = format_dict_date_to_list_for_plot(TDG_aaa_dt)

model_result = W2_generic_read(model_dir+projects[project]['deep_outlet_file'], 0, print_errors=True, csv = True)
TDG_aaa_dt = {}
for jday in model_result.keys():
    dt = datetime(year-1,12,31)+timedelta(jday)
    TDG_aaa_dt[dt] = model_result[jday]['  TDG(%)']
model_dt, model_TDGdeep = format_dict_date_to_list_for_plot(TDG_aaa_dt)
...
ax1.fill_between(model_dt[1:], model_TDGdeep[1:], model_TDGsurface[1:], color = 'k', alpha=0.3, edgecolor='none', lw = 0.0, label = 'Model vertical range')
